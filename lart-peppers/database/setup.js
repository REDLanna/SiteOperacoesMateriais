// database/setup.js — Cria e popula o banco de dados SQLite
'use strict';
...
try {
  ...
  try {
    ...
    console.error('❌  SQLite indisponível. Use Node >= 22.5 ou instale better-sqlite3.');
    process.exit(1);
  }
}
...
db.exec(`
  CREATE TABLE IF NOT EXISTS products (
    ...
    name TEXT NOT NULL, subtitle TEXT, description TEXT, volume_ml INTEGER,
    price REAL NOT NULL, stock INTEGER DEFAULT 10, shu TEXT, ingredients TEXT,
    active INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS customers (
    id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL,
    email TEXT UNIQUE NOT NULL, phone TEXT, cpf TEXT, password_hash TEXT,
    created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS addresses (
    id INTEGER PRIMARY KEY AUTOINCREMENT, customer_id INTEGER NOT NULL REFERENCES customers(id),
    label TEXT DEFAULT 'Principal', cep TEXT, street TEXT, number TEXT,
    complement TEXT, district TEXT, city TEXT, state TEXT,
    created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS orders (
    ...
    customer_phone TEXT, customer_cpf TEXT,
    ...
    order_status TEXT DEFAULT 'received', notes TEXT,
    created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS order_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER NOT NULL REFERENCES orders(id),
    product_id INTEGER NOT NULL REFERENCES products(id),
    product_name TEXT NOT NULL, volume_ml INTEGER,
    quantity INTEGER NOT NULL, unit_price REAL NOT NULL, total_price REAL NOT NULL
  );
  CREATE TABLE IF NOT EXISTS payments (
    id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER NOT NULL REFERENCES orders(id),
    method TEXT NOT NULL, card_last4 TEXT, card_brand TEXT, cardholder_name TEXT,
    installments INTEGER DEFAULT 1, amount REAL NOT NULL,
    status TEXT DEFAULT 'pending', transaction_id TEXT,
    created_at TEXT DEFAULT (datetime('now'))
  );
...
if ((prodCount?.n || 0) === 0) {
  const ins = (slug,name,sub,desc,vol,price,stock,shu,ing) =>
    run('INSERT INTO products (slug,name,subtitle,description,volume_ml,price,stock,shu,ingredients) VALUES (?,?,?,?,?,?,?,?,?)',
      slug,name,sub,desc,vol,price,stock,shu,ing);

  ins('reaper-30ml','Reaper Concentrado','Molho Extremo 30ml',
    ...
    30,48.00,25,'2.200.000 SHU','Pimenta Carolina Reaper, azeite extravirgem, vinagre branco, sal marinho, açúcar.');

  ins('reaper-100ml','Reaper Premium','Molho Extremo 100ml',
    ...
    100,65.00,15,'2.200.000 SHU','Pimenta Carolina Reaper, azeite extravirgem, vinagre branco, sal marinho, açúcar.');

  ins('reaper-kit','Kit L\'Art','Kit Completo 30ml + 100ml',
    ...
    null,105.00,10,'2.200.000 SHU','1x Reaper Concentrado 30ml + 1x Reaper Premium 100ml');
